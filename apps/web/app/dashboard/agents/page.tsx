...
function HealthBadge({ status }: { status: "healthy" | "warning" | "critical" }) {
  const styles = {
    ...
    warning: "bg-yellow-900/40 text-yellow-400 border-yellow-800 badge-warning",
    critical: "bg-red-900/40 text-red-400 border-red-800 badge-critical",
  };
  ...
  return (
    <div className="animate-fade-in-up">
      <div className="flex items-center justify-between mb-1">
        <h1 className="text-2xl font-bold text-gradient-purple">AI Workforce</h1>
        <Link
          href={`/dashboard/agents/compare`}
          className="px-3 py-1.5 text-sm rounded-lg border border-purple-800 bg-purple-900/30 text-purple-300 hover:bg-purple-900/60 btn-glow transition-all"
        >
          Compare
        </Link>
      </div>
      <p className="text-gray-500 text-sm mb-6">All connected AI agents and their health.</p>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-gray-800 bg-gray-900/80">
                ...
                <th className="text-left px-4 py-3 text-gray-400 font-medium text-xs uppercase tracking-wide">Platform</th>
                <th className="text-left px-4 py-3 text-gray-400 font-medium text-xs uppercase tracking-wide">Health</th>
                <th className="text-right px-4 py-3 text-gray-400 font-medium text-xs uppercase tracking-wide">Traces</th>
                <th className="text-right px-4 py-3 text-gray-400 font-medium text-xs uppercase tracking-wide">Open Incidents</th>
                <th className="text-left px-4 py-3 text-gray-400 font-medium text-xs uppercase tracking-wide">Top Issue</th>
                <th className="text-left px-4 py-3 text-gray-400 font-medium text-xs uppercase tracking-wide">Last Active</th>
              </tr>
            </thead>
            <tbody>
              {(agents as AgentRow[]).map((agent, index) => {
                ...
                return (
                  ...
                  >
                    ...
                    <td className="px-4 py-3 text-right text-gray-400 tabular-nums">{agent._count.traces}</td>
                    <td className="px-4 py-3 text-right">
                      <span className={openIncidents > 0 ? "text-yellow-400 font-medium tabular-nums" : "text-gray-600 tabular-nums"}>
                        {openIncidents}
                      </span>
                    </td>
                    <td className="px-4 py-3 text-gray-400">
                      {topCategory ? CATEGORY_LABELS[topCategory] : <span className="text-gray-700">—</span>}
                    </td>
